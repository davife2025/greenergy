...
exports.energyRequestsRoutes = energyRequestsRoutes;
...
const createRequestSchema = zod_1.z.object({
    solarProfileId: zod_1.z.string().uuid(),
});
async function energyRequestsRoutes(app) {
    ...
    app.post("/energy-requests", async (request, reply) => {
        ...
        const platformCommissionNgn = Number((amountNgn * pricing_js_1.MARKETPLACE_COMMISSION_RATE).toFixed(2));
        const hostPayoutNgn = Number((amountNgn - platformCommissionNgn).toFixed(2));
        const { data: energyRequest, error: insertError } = await supabase_js_1.supabase
        ...
        const { data: seeker } = await supabase_js_1.supabase
            .from("users")
        ...
        try {
            ...
            const { authorizationUrl, reference } = await (0, payment_collection_js_1.initializeTransaction)({
                email,
                amountNgn,
            ...
            // start the checkout.
            await supabase_js_1.supabase.from("energy_requests").delete().eq("id", energyRequest.id);
    ...
    app.post("/energy-requests/:id/confirm", async (request, reply) => {
        ...
        const { data: energyRequest, error: fetchError } = await supabase_js_1.supabase
            .from("energy_requests")
            .select("*")
            .eq("id", id)
            .single();
        ...
        const { data: host } = await supabase_js_1.supabase
            .from("users")
        ...
        if (!(0, payout_provider_js_1.isSupportedPayoutProvider)(host.mobile_money_provider)) {
        ...
        try {
            ...
            await (0, payout_provider_js_1.initiateTransfer)({
            ...
            await supabase_js_1.supabase.from("energy_requests").update({ status: "paid_out" }).eq("id", id);
