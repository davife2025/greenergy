...
async function main() {
    ...
    check("Using pnpm (not npm/yarn)", hasPnpmLock && !hasNpmLock && !hasYarnLock, hasNpmLock
    ...
    check("Node.js version >= 20", nodeMajor >= 20, `Running Node ${process.versions.node}${nodeMajor < 20 ? " — this repo requires Node 20+" : ""}`);
    ...
    if (paystackKey) {
        try {
            ...
            if (res.status === 401 || res.status === 403) {
                ...
                check("Paystack key valid", true, `Key works. ${isTestKey ? "This is a TEST key — no real money will move. Switch to sk_live_... when ready for real transactions." : "⚠ This is a LIVE key — real money WILL move."}`);
            }
        }
        catch (err) {
