...
import { batchValueNgn, PAYOUT_USER_SHARE } from "../lib/pricing.js";
import {
...
export async function payoutsRoutes(app: FastifyInstance) {
  ...
  app.post("/admin/payouts/process", async (_request, reply) => {
    ...
    if (batchesError) {
      return reply.status(500).send({ status: "error", message: batchesError.message });
    }

    if (!batches || batches.length === 0) {
      return reply.status(200).send({ status: "ok", message: "No verified batches ready for payout." });
    }
    ...
    return reply.status(200).send({ status: "ok", batches: results });
  });
}
...
async function payUser(userId: string, carbonBatchId: string, amountNgn: number) {
  ...
  // Always create the payout record, even if we can't send it yet — it
  // should be visible in the dashboard as owed, not silently dropped.
